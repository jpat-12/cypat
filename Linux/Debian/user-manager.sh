...
users_to_delete=$(cat "$user_dir/users-all.txt")
...
echo "Printing users to $user_dir/users-all2.txt"
sleep 2
...
for user in $users; do
  echo $user >> "$user_dir/users-all2.txt"  
done

echo "Removing default users from $user_dir/users-all2.txt"
sleep 2
# Get a list of all users from $user_dir/users-default.txt
users_to_remove=$(cat "$user_dir/users-default.txt")

# Iterate over the list of users and remove them from $user_dir/users-all2.txt
for user in $users_to_remove; do
  # Remove the user from $user_dir/users-all2.txt
  sed -i "/^$user$/d" "$user_dir/users-all2.txt"
done
...
nano "$user_dir/user-current.txt"

echo "The users at $user_dir/users-all2.txt will be deleted" 
echo "This script will open that file, when you close it the users listed will be deleted" 
echo "Please make sure the users inside actually need to be deleted."

nano "$user_dir/users-all2.txt"

echo "removing current user from $user_dir/users-all2.txt"
sleep 2
# Get a list of all users from $user_dir/users-authorized.txt
users_to_remove=$(cat "$user_dir/user-current.txt")
...
while true; do
  ...
  if [ "$add_user" == "y" ]; then
    ...
    sudo usermod -aG $new_group1 $new_user
